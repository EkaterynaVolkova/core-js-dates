...
function getNextFriday(date) {
  ...
  const days = day < 5 ? 5 - day : 12 - day;
  const futureDate = new Date();
  futureDate.setTime(date.getTime());
  const time = days * 24 * 60 * 60 * 1000;
  futureDate.setTime(futureDate.getTime() + time);
  return futureDate;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const daysLeft = (end.getTime() - start.getTime()) / msPerDay;
  return Math.ceil(daysLeft) + 1;
}
...
function getWeekNumberByDate(date) {
  ...
  return Math.ceil(((dateUTC - yearStart) / oneDay + 1) / 7);
}
